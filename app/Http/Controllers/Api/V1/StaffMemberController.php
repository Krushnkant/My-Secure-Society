...
class StaffMemberController extends BaseController
{
    ...
    public function save_staff_member_duty_area(Request $request)
    {
        ...
        // Validation rules
        $rules = [
            'staff_member_id' => 'required|integer|exists:society_staff_member,society_staff_member_id,deleted_at,NULL,society_id,'.$society_id,
        ...
        if ($request->has('duty_area.*.duty_area_time_id') && $request->input('duty_area.*.duty_area_time_id') != 0) {
            $rules['duty_area.*.duty_area_time_id'] .= '|exists:staff_duty_area_time,staff_duty_area_time_id,deleted_at,NULL';
        }
        ...
        try {
            ...
            foreach ($request->duty_area as $area) {
                if ($area['is_deleted'] == 2) {
                    ...
                    $areaTime->save();
                } elseif ($area['is_deleted'] == 1 && isset($area['duty_area_time_id'])) {
        ...
        return $this->sendResponseWithData($data, 'Staff Duty Area successfully');
            ...
            DB::rollBack();
            return $this->sendError(500, 'An error occurred while saving the Staff Duty Area.', "Internal Server Error", []);
        }
    }
    ...
    {
        $validator = Validator::make($request->all(), [
            'duty_area_time_id' => 'required|exists:staff_duty_area_time,staff_duty_area_time_id,deleted_at,NULL',
        ]);
        ...
        $staff = StaffDutyAreaTime::with('user')->find($request->duty_area_time_id);
        ...
        $data = array();
        $temp['duty_area_time_id'] = $staff->staff_duty_area_time_id;
